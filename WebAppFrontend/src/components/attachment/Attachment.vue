...
onMounted(async () => {
  try {
    if (mode === AttachmentMode.Edit) file.value = await fileService.getFile(fileId);
  } catch (error) {
    console.log(error);
...
<template>
  <template v-if="file">
    <div class="field flex align-items-center" style="justify-content: space-between">
      ...
      <template v-else>
        <Button v-if="file.telegramUserId === userProfile.id" 
					class="flex-end" 
					icon="pi pi-pencil" 
					severity="secondary" 
					rounded 
    ...
    <div class="field">
      <div class="flex justify-content-center">
        ...
        <Skeleton v-if="!imageLoaded && file.fileType !== 'Other'" 
					width="300px" 
					height="300px">
				</Skeleton>
      </div>
        ...
        <div class="flex flex-row justify-between items-start gap-2">
          <div>
            ...
            <div class="flex flex-wrap gap-1 mt-2">
              <Chip v-for="keyWord in file.keyWords" 
								:key="keyWord" 
								:label="keyWord" 
							/>
            </div>
